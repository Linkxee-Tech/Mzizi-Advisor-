...
const Dashboard: React.FC<DashboardProps> = ({ profile }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [fetchedTools, fetchedInsights] = await Promise.all([
                    getTools(),
                    getInsights(profile.id)
                ]);
                setTools(fetchedTools);
                // Transform insights to match chart data format if needed, but getInsights already maps it
                setChartData(fetchedInsights);
            } catch (error) {
