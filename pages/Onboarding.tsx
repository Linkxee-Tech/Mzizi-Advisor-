...
import { BusinessProfile, Currency, PersonaId } from '../types';
...
import { getCurrencies, getProfiles, createProfile } from '../services/api';
...
const COUNTRY_TO_CURRENCY_CODE: Record<string, string> = {
    "Nigeria": "NGN", "Kenya": "KES", "South Africa": "ZAR", "Ghana": "GHS", "United States": "USD", "Tanzania": "TZS", "Uganda": "UGX", "Rwanda": "RWF"
};
...
const Onboarding: React.FC<OnboardingProps> = ({ setProfile }) => {
    ...
    const [demoProfiles, setDemoProfiles] = useState<BusinessProfile[]>([]);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const [fetchedCurrencies, fetchedProfiles] = await Promise.all([
                    getCurrencies(),
                    getProfiles()
                ]);
                setCurrencies(fetchedCurrencies);
                setDemoProfiles(fetchedProfiles);
            } catch (error) {
    ...
    const handleSelectPersona = (profile: BusinessProfile) => {
        setProfile(profile);
        navigate('/');
    };
    ...
    const handleGoogleLogin = async () => {
        ...
        // This part would ideally create a profile via API too if not exists
        const googleProfileData: Partial<BusinessProfile> = {
            ownerName: 'Alex Doe',
        ...
        try {
            const created = await createProfile(googleProfileData);
            setProfile(created);
            navigate('/');
        } catch (e) {
    ...
    const handleEmailLogin = async (e: React.FormEvent) => {
        ...
        await simulateAuthDelay();
        // Mock login by creating/updating a profile for now
        const userProfileData: Partial<BusinessProfile> = {
        ...
        try {
            const created = await createProfile(userProfileData);
            setProfile(created);
            navigate('/');
        } catch (e) {
            console.error("Login failed", e);
    ...
    const handleSignupSubmit = async () => {
        ...
        const newProfileData: Partial<BusinessProfile> = {
                ...
                <div className="relative">
                    <select
                        ...
                        onChange={e => {
                            ...
                            const code = COUNTRY_TO_CURRENCY_CODE[newCountry] || 'USD';
                            const currencyObj = currencies.find(c => c.code === code);
                            const newCurrency = currencyObj ? currencyObj.symbol : '$';
                    ...
                    {BUSINESS_GOALS.map(goal => {
                        ...
                        return (
                            <button
                                ...
                                className={`p-3 rounded-xl text-left text-sm font-medium transition-all border ${isSelected
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex gap-1.5">
                    {[1, 2, 3, 4].map(step => (
                        <div
                            key={step}
                            className={`h-1.5 w-6 rounded-full transition-all duration-300 ${step <= signupStep ? 'bg-green-600' : 'bg-gray-200 dark:bg-gray-700'
                                }`}
                        />
                    ))}
                </div>
