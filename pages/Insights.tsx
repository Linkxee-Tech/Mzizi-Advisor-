...
import { BusinessProfile, ChartData } from '../types';
...
const Insights: React.FC<InsightsProps> = ({ profile }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const insights = await getInsights(profile.id);
        setData(insights);
      } catch (error) {
      ...
      <div className="bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 transition-colors">
        ...
        <div className="h-64 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.length > 0 ? data : []} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
